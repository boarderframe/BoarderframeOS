...
def fix_metric_displays():
    ...
    if welcome_matches:
        print(
            f"   Found {len(welcome_matches)} instances with values: {welcome_matches}"
        )
    ...
    if re.search(division_pattern, content):
        content = re.sub(
            division_pattern, r'\1{self.get_metric("divisions", "total")}\2', content
        )
    ...
    if refresh_method in content:
        ...
        if (
            "self._get_centralized_metrics()"
            not in content[refresh_start : refresh_start + 1000]
        ):
            indent = "        "
    ...
    replacements = [
        ...
        (
            r"(>\s*)(2)(\s*</[^>]+>\s*<[^>]+>Agents)",
            r'\1{self.get_metric("agents", "total")}\3',
        ),
        (
            r"(>\s*)(45)(\s*</[^>]+>\s*<[^>]+>Departments)",
            r'\1{self.get_metric("departments", "total")}\3',
        ),
        (
            r"(>\s*)(9)(\s*</[^>]+>\s*<[^>]+>Divisions)",
            r'\1{self.get_metric("divisions", "total")}\3',
        ),
    ]
