...
def fix_all_js_issues():
    ...
    for i, script in enumerate(script_blocks):
        if "{{" in script:
            ...
            fixed_script = re.sub(
                r"\$\{\{(\w+)\}\}", r"${$1}", fixed_script
            )  # Fix template literals
            fixed_script = re.sub(
                r"`([^`]*)\{\{(\w+)\}\}([^`]*)`", r"`$1${$2}$3`", fixed_script
            )  # Fix in template strings
            fixed_script = re.sub(
                r"function (\w+)\(\) \{\{", r"function $1() {", fixed_script
            ...
            content = content.replace(
                f"<script>{script}</script>", f"<script>{fixed_script}</script>"
            )
    ...
    countdown_match = re.search(
        r"function startRefreshCountdown\(\) \{([^}]+)\}", content
    )
    if countdown_match:
        ...
        if update_display_pos > countdown_pos or update_display_pos == -1:
            print(
                "❌ updateRefreshDisplay is defined after startRefreshCountdown or missing"
            )
    ...
    if update_display_count > 1:
        ...
        if first_pos > -1:
            ...
            remaining = re.sub(
                r"function updateRefreshDisplay\([^)]*\) \{[^}]+\}\s*", "", remaining
            )
    ...
    if "let refreshCounter" not in content and "var refreshCounter" not in content:
        ...
        if (
            first_script > 8
            and "refreshCounter = 30" not in content[: first_script + 1000]
        ):
            content = (
