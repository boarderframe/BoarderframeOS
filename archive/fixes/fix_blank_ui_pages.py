...
def fix_corporate_headquarters():
    ...
    if "_generate_enhanced_agents_html()" not in current_agents_content:
        ...
        metrics_card_end = current_agents_content.rfind(
            "</div>", 0, current_agents_content.find("<!-- Organizational Chart -->")
        )
        ...
        if metrics_card_end == -1:
            ...
            insert_pos = (
                current_agents_content.find(
                    "</div>", current_agents_content.find("get_agent_metrics_cards()")
                )
                + 6
            )
        ...
        new_agents_content = (
            current_agents_content[:insert_pos]
            + """
            </div>
        ...
        # Replace the content
        content = (
            content[:agents_tab_start] + new_agents_content + content[agents_tab_end:]
        )
