...
def fix_refresh_modal_issues():
    ...
    pattern1 = (
        r'<button id="closeRefreshBtn" onclick="closeModal\(\'globalRefreshModal\'\)"'
    )
    replacement1 = (
        '<button id="closeRefreshBtnHeader" onclick="closeGlobalRefreshModal()"'
    )
    ...
    pattern5 = (
        r'(async def _collect_organizational_metrics\(self\):[\s\S]*?"""[\s\S]*?try:)'
    )
