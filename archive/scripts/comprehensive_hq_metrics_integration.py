...
from core.hq_metrics_layer import BFColors, BFIcons, MetricValue"""
    ...
    overview_widgets_pattern = (
        r'(<h3>System Status</h3>)([\s\S]*?)(<div class="widget-grid">[\s\S]*?</div>)'
    )
    ...
    content = re.sub(
        overview_widgets_pattern, overview_replacement, content, flags=re.DOTALL
    )
    ...
    if "self.metrics_layer.get_agent_cards_html()" not in content:
        ...
        agents_section = content[
            content.find('id="agents"') : content.find('id="leaders"')
        ]
    ...
    monitor_pattern = (
        r"(def monitor_system\(self\):[\s\S]*?while self\.monitoring_active:)"
    )
    ...
    dept_display_pattern = (
        r'<div class="widget-value"[^>]*>\s*{total_departments}\s*</div>'
    )
