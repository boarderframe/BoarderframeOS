...
async def test_panel():
    ...
    print(
        f"   - Department structure loaded: {'boarderframeos_departments' in panel.department_structure}"
    )
    ...
    if agents_found:
        ...
        for agent in agents_found[:5]:
            ...
            print(
                f"     • {agent['name']} ({agent['title']}) - {agent['department']} - Tier: {tier}"
            )
    ...
    for tier, config in tiers.items():
        print(
            f"   - {tier.upper()}: {config['default_provider']}/{config['default_model']}"
        )
