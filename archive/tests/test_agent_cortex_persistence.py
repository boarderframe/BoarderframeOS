...
async def test_persistence_flow():
    ...
    try:
        await ui._save_cortex_setting(
            "current_strategy", "cost_optimized", "string", "Test strategy setting"
        )
        ...
        config = await ui._load_config_from_db()
        if (
            config["cortex_settings"].get("current_strategy", {}).get("value")
            == "cost_optimized"
        ):
    ...
    try:
        ...
        if (
            ui.cortex_config["agent_configurations"]
            .get("test_agent", {})
            .get("quality_threshold")
            == 0.95
        ):
