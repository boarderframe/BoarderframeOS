...
class RegistryTestSuite:
        ...
        for i in range(10):
            ...
            agent = await self.client.register_agent(
                ...
                capabilities=(
                    ...
                    if i % 2 == 0
                    else ["support", "communicate"]
                ),
                department_id=f"{dept}-dept",
                llm_model="gpt-4" if i % 3 == 0 else "gpt-3.5-turbo",
                metadata={"test": True, "index": i, "created_by": "test_suite"},
            )
            agents.append(agent)
    ...
    async def test_department_registration(self):
        ...
        for dept_name in ["Sales", "Analytics", "Engineering", "Support"]:
            dept = await self.client.register_department(
                ...
                capabilities=[
                    f"{dept_name.lower()}_analysis",
                    f"{dept_name.lower()}_reporting",
                ],
    ...
    async def test_heartbeat_and_health(self):
        ...
        agent = await self.client.register_agent(
            name="HeartbeatTestAgent", capabilities=["test"], metadata={"test": True}
        )
        self.test_entities.append(agent.id)
    ...
    async def test_event_subscriptions(self):
        ...
        async def on_event(event):
            ...
            logger.info(
                f"Received event: {event.event_type} for {event.entity_type} {event.entity_id}"
            )
    ...
    async def test_statistics(self):
        ...
        logger.info(
            f"Average health score: {stats['performance']['average_health_score']:.2f}"
        )
        logger.info(
            f"Online percentage: {stats['performance']['online_percentage']:.2f}%"
        )
    ...
    async def test_performance(self):
        ...
        logger.info(
            f"Discovered {len(discovered)} agents in {discovery_time*1000:.2f} ms"
        )

        # Measure batch update time
        start = datetime.utcnow()
        for agent_id in agents[:10]:
            await self.client.update_entity(
                agent_id,
                {
