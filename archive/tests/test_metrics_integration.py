...
def test_metrics():
    ...
    try:
        ...
        print(f"\n   Agents:")
        print(
            f"     - Online in DB: {metrics.get_metric_value('agents', 'summary.online', 0)}"
        )
        print(
            f"     - Operational: {metrics.get_metric_value('agents', 'summary.operational', 0)}"
        )
        print(
            f"     - Deployed: {metrics.get_metric_value('agents', 'summary.deployed', 0)}"
        )
        ...
        print(f"\n   Departments:")
        print(
            f"     - Total: {metrics.get_metric_value('departments', 'summary.total', 0)}"
        )
        print(
            f"     - Active: {metrics.get_metric_value('departments', 'summary.active', 0)}"
        )
        ...
        print(f"\n   Servers:")
        print(
            f"     - Total: {metrics.get_metric_value('servers', 'summary.total', 0)}"
        )
        print(
            f"     - Healthy: {metrics.get_metric_value('servers', 'summary.healthy', 0)}"
        )
