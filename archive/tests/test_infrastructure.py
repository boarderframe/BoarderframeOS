...
class InfrastructureTestSuite:
        ...
        try:
            ...
            try:
                await redis.xgroup_create(
                    stream_name, "test_group", id="0", mkstream=True
                )
    ...
    async def test_memory_usage(self) -> Dict[str, Any]:
        ...
        conn_pool = await asyncpg.create_pool(
            self.postgres_dsn, min_size=5, max_size=10
        )
        try:
        ...
        return {
            ...
            "connection_pool_overhead_mb": round(
                (pool_memory_after - pool_memory_before) / 1024 / 1024, 2
            ),
            "memory_status": (
                "acceptable" if memory_info.rss < 500 * 1024 * 1024 else "high"
            ),
        }
...
async def main():
    ...
    for test_name, result in results.items():
        ...
        print(
            f"{status_icon} {test_name:<25} | {result['status']:<6} | {duration:6.3f}s"
        )
