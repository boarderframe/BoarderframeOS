...
def verify_fixes():
    ...
    if (
        'id="closeRefreshBtnHeader"' in content
        and 'onclick="closeGlobalRefreshModal()"' in content
    ):
    ...
    if (
        "await loop.run_in_executor(None, self.dashboard._fetch_organizational_data)"
        in content
    ):
    ...
    if (
        "failed_components" in content
        and "updateComponentStatus(component, 'error')" in content
    ):
    ...
    if close_btn_count > 1:
        print(
            f"⚠️  Found {close_btn_count} elements with id='closeRefreshBtn' - potential conflict!"
        )
    else:
