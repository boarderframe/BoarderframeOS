...
def fix_nested_tabs():
    ...
    if match:
        ...
        nested_count = agents_section.count('class="tab-content"') - 1  # -1 for the main div
        ...
        if first_occurrence > -1:
            ...
            before_first = modified_section[:first_occurrence + 19]  # +19 for 'class="tab-content">'
            after_first = modified_section[first_occurrence + 19:]
            
            # Replace all tab-content in the after_first part
            after_first = after_first.replace('class="tab-content"', 'class="agent-tab-content"')
            after_first = after_first.replace('class="tab-content active"', 'class="agent-tab-content active"')
        ...
        if css_insert_point > -1:
            ...
            content = content[:css_insert_point] + css_addition + content[css_insert_point:]
    ...
    if 'showAgentTab' in content:
        ...
        content = content.replace("querySelectorAll('.tab-content')", "querySelectorAll('.agent-tab-content')")
        content = content.replace('getElementById(tabName).querySelector(".tab-content")', 'getElementById(tabName).querySelector(".agent-tab-content")')
