...
def test_refresh_api():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"📋 Refreshed components: {result.get('result', {}).get('refreshed_components', [])}")
...
def check_corporate_hq():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Corporate Headquarters returned status code: {response.status_code}")
            return False
    except requests.exceptions.RequestException as e:
...
def main():
    ...
    if not check_corporate_hq():
        print("\n💡 Instructions:")
        print("1. Make sure corporate headquarters is running: python corporate_headquarters.py")
    ...
    if test_refresh_api():
        ...
        print("   - mcp_servers → registry_data → departments_data → organizational_data")
    else:
