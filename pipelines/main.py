...
async def filter_inlet(pipeline_id: str, form_data: FilterForm):
    # Strip 'pipeline.' prefix if present to get the actual module name
    actual_pipeline_id = pipeline_id.replace("pipeline.", "") if pipeline_id.startswith("pipeline.") else pipeline_id
    
    # Check if pipeline_id exists directly or if it's a manifold pipeline
    manifold_pipeline_id = None
    if pipeline_id not in app.state.PIPELINES:
        # Check if this is a manifold pipeline by looking for any pipeline starting with pipeline_id
        manifold_matches = [pid for pid in app.state.PIPELINES.keys() if pid.startswith(f"{pipeline_id}.")]
        if not manifold_matches:
            # Also check if this is a filter pipeline with the 'pipeline.' prefix
            if not pipeline_id.startswith("pipeline.") or actual_pipeline_id not in PIPELINE_MODULES:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
                    detail=f"Filter {pipeline_id} not found",
                )
        # For manifold, we'll use the base pipeline_id from PIPELINE_MODULES

    try:
        pipeline = app.state.PIPELINES[form_data.body["model"]]
        if pipeline["type"] == "manifold":
            actual_pipeline_id = pipeline_id.split(".")[0]
    except:
        pass

    pipeline = PIPELINE_MODULES[actual_pipeline_id]
