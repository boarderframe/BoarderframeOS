...
class ExtractRequest(BaseModel):
    ...
    attribute: Optional[str] = Field(
        None, description="Attribute to extract (default: text)"
    )
        ...
        try:
            ...
            if request.wait_for_selector:
                await self.page.wait_for_selector(
                    request.wait_for_selector, timeout=request.timeout
                )
        ...
        try:
            ...
            if request.multiple:
                ...
                if request.attribute:
                    data = [
                        await el.get_attribute(request.attribute) for el in elements
                    ]
                else:
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "browser_server:app", host=args.host, port=args.port, reload=args.reload
    )
