...
class PostgreSQLQueryCache:
    ...
    def _should_cache_query(self, sql: str) -> bool:
        ...
        return (
            ...
            and "RANDOM()" not in sql_upper
            and "NOW()" not in sql_upper
            and "CURRENT_TIMESTAMP" not in sql_upper
        )
...
class InsertRequest(BaseModel):
    ...
    on_conflict: str = Field(
        "DO NOTHING", description="Conflict resolution: DO NOTHING, DO UPDATE"
    )
...
class VectorSearchRequest(BaseModel):
    ...
    similarity_threshold: float = Field(
        default=0.8, description="Minimum similarity threshold"
    )
    limit: int = Field(default=10, description="Maximum results to return")
    agent_id: Optional[str] = Field(
        default=None, description="Filter by specific agent"
    )
...
class PostgreSQLManager:
        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                ...
                if execution_time > 1000:
                    logger.warning(
                        f"Slow query ({execution_time:.2f}ms): {request.sql[:100]}..."
                    )
            ...
            async with self.pool.acquire() as conn:
                ...
                if self.redis and rows_affected > 0:
                    await self._publish_event(
                        "insert", request.table, data or request.data
                    )
    ...
    async def update_data(self, request: UpdateRequest) -> DatabaseResponse:
        ...
        try:
            ...
            set_clauses = [
                f"{col} = ${i+1}" for i, col in enumerate(request.data.keys())
            ]
            where_clauses = [
                f"{col} = ${i+len(request.data)+1}"
                for i, col in enumerate(request.where.keys())
            ]
            ...
            async with self.pool.acquire() as conn:
                ...
                if self.redis and rows_affected > 0:
                    await self._publish_event(
                        "update", request.table, {**request.where, **request.data}
                    )
    ...
    async def delete_data(self, request: DeleteRequest) -> DatabaseResponse:
        ...
        try:
            ...
            where_clauses = [
                f"{col} = ${i+1}" for i, col in enumerate(request.where.keys())
            ]
            where_values = list(request.where.values())
    ...
    async def insert_memory(self, request: MemoryInsertRequest) -> DatabaseResponse:
        ...
        try:
            ...
            if embedding is None:
                ...
                logger.warning(
                    "Using random embedding - implement actual embedding generation"
                )
...
async def execute_query(
    request: QueryRequest, db: PostgreSQLManager = Depends(get_db_manager)
):
...
async def insert_data(
    request: InsertRequest, db: PostgreSQLManager = Depends(get_db_manager)
):
    """Insert data with enhanced conflict handling"""
...
async def update_data(
    request: UpdateRequest, db: PostgreSQLManager = Depends(get_db_manager)
):
    """Update data with change tracking"""
...
async def delete_data(
    request: DeleteRequest, db: PostgreSQLManager = Depends(get_db_manager)
):
    """Delete data with audit logging"""
...
async def vector_similarity_search(
    request: VectorSearchRequest, db: PostgreSQLManager = Depends(get_db_manager)
):
...
async def insert_memory(
    request: MemoryInsertRequest, db: PostgreSQLManager = Depends(get_db_manager)
):
    """Insert agent memory with automatic embedding generation"""
...
async def get_database_stats(db: PostgreSQLManager = Depends(get_db_manager)):
    ...
    try:
        ...
        return {
            "database_size": (
                size_result.data.get("size") if size_result.success else "unknown"
            ),
...
async def get_recent_events(
    limit: int = 10, db: PostgreSQLManager = Depends(get_db_manager)
):
...
async def get_performance_stats(db: PostgreSQLManager = Depends(get_db_manager)):
    ...
    try:
        ...
        if (
            pg_stats
            and pg_stats.get("blocks_read", 0) + pg_stats.get("blocks_hit", 0) > 0
        ):
            cache_hit_ratio = pg_stats["blocks_hit"] / (
                pg_stats["blocks_read"] + pg_stats["blocks_hit"]
            )
        else:
            cache_hit_ratio = 0
