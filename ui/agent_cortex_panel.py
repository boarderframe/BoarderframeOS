...
class AgentCortexPanel:
    ...
    def _setup_routes(self):
        ...
        def get_overview():
            ...
            overview = {
                ...
                "active_providers": len(
                    [p for p in self.llm_providers.values() if p["is_active"]]
                ),
                "departments": len(
                    self.department_structure.get("boarderframeos_departments", {})
                    .get("phase_1_trinity", {})
                    .get("departments", {})
                ),
        ...
        @self.app.route(
            "/api/cortex/providers/<provider_name>", methods=["GET", "PUT", "DELETE"]
        )
        ...
        def get_agents():
            ...
            departments = self.department_structure.get(
                "boarderframeos_departments", {}
            )
                    ...
                    agents.append(
                        {
                            ...
                            "provider": assignment.get(
                                "provider", config.get("provider", "not_assigned")
                            ),
                            "model": assignment.get(
                                "model", config.get("model", "not_assigned")
                            ),
                            "temperature": assignment.get(
                                "temperature", config.get("temperature", 0.7)
                            ),
                            "max_tokens": assignment.get(
            ...
            if request.method == "GET":
                ...
                try:
                    ...
                    return jsonify(
                        {
                            "status": "success",
                            "message": f"LLM configuration for {agent_name} updated",
                        }
                    )
                finally:
                    loop.close()
        ...
        def update_tier(tier):
            ...
            try:
                ...
                return jsonify(
                    {"status": "success", "message": f"Tier {tier} defaults updated"}
                )
            finally:
                loop.close()
        ...
        def test_llm():
            ...
            try:
                result = loop.run_until_complete(
                    self._test_llm_connection(provider, model)
                )
        ...
        def get_agent_performance(agent_name):
            ...
            try:
                metrics = loop.run_until_complete(
                    self._get_agent_performance(agent_name)
                )
    ...
    def _determine_tier(self, title: str) -> str:
        ...
        if any(word in title_lower for word in ["ceo", "chief", "digital twin"]):
            ...
            word in title_lower for word in ["head", "director", "manager", "officer"]
        ):
            return "department"
    ...
    async def _delete_provider(self, provider_name: str):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
                "DELETE FROM llm_providers WHERE provider_name = ?", (provider_name,)
            )
            await db.commit()
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            """,
                (
                    ...
                    data["default_model"],
                    data.get("budget_provider"),
                    data.get("budget_model"),
                    data.get("local_provider"),
                    data.get("local_model"),
    ...
    def run(self):
        ...
        print(
            f"🧠 Starting Agent Cortex Management Panel on http://localhost:{self.port}"
        )
        print("=" * 60)
        print("Features:")
