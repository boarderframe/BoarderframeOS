...
class AgentCortexManagementUI:
    ...
    async def _load_cortex_configuration(self):
        ...
        try:
            ...
            if db_config["model_registry"]:
                ...
                self.cortex.model_selector.model_registry.update(
                    db_config["model_registry"]
                )

            if db_config["agent_configurations"]:
                self.cortex_config["agent_configurations"].update(
                    db_config["agent_configurations"]
                )

            if db_config["cortex_settings"].get("current_strategy"):
                strategy_value = db_config["cortex_settings"]["current_strategy"][
                    "value"
                ]
    ...
    def _setup_routes(self):
        ...
        def manage_models():
            ...
            if request.method == "GET":
                ...
                model_type = updates.get(
                    "model_type"
                )  # primary, fallback, budget, local
                model_config = updates.get("config")

                if tier and model_type and model_config:
                    # Update in-memory config
                    self.cortex_config["model_registry"][tier][
                        model_type
                    ] = model_config
                    # Apply to Agent Cortex
                    self.cortex.model_selector.model_registry[tier][
                        model_type
                    ] = model_config
                    ...
                    try:
                        loop.run_until_complete(
                            self._save_model_config_to_db(
                                tier, model_type, model_config
                            )
                        )
                        return jsonify(
                            {
                ...
                return (
                    jsonify({"status": "error", "message": "Invalid configuration"}),
                    400,
                )
        ...
        def manage_strategy():
            ...
            if request.method == "GET":
                ...
                try:
                    ...
                    try:
                        loop.run_until_complete(
                            self._save_cortex_setting(
                                "current_strategy",
                                strategy_name,
                                "string",
                                "Global Agent Cortex selection strategy",
                            )
                        )
                        return jsonify(
                            {
                    ...
                    return (
                        jsonify({"status": "error", "message": "Invalid strategy"}),
                        400,
                    )

        @self.app.route(
            "/api/agent-cortex/agents/<agent_name>/config", methods=["GET", "POST"]
        )
        ...
        def get_performance():
            ...
            try:
                ...
                for (
                    tracking_id,
                    data,
                ) in self.cortex.performance_analyzer.performance_data.items():
                ...
                return jsonify(
                    {
                        ...
                        "summary": {
                            ...
                            "avg_cost": (
                                sum(p["cost"] for p in performance_data)
                                / len(performance_data)
                                if performance_data
                                else 0
                            ),
                            "avg_latency": (
                                sum(p["latency"] for p in performance_data)
                                / len(performance_data)
                                if performance_data
                                else 0
                            ),
                            "avg_quality": (
                                sum(p["quality"] for p in performance_data)
                                / len(performance_data)
                                if performance_data
                                else 0
                            ),
            ...
            try:
                ...
                selection = loop.run_until_complete(
                    self.cortex.model_selector.select_optimal_model(
                        test_request,
                        SelectionStrategy(
                            data.get("strategy", self.cortex.current_strategy.value)
                        ),
                    )
                )
        ...
        def import_config():
            ...
            try:
                ...
                self.cortex.model_selector.model_registry = config_data[
                    "model_registry"
                ]

                if "strategy_settings" in config_data:
                    strategy_name = config_data["strategy_settings"].get(
                        "current_strategy"
                    )
                    if strategy_name:
                ...
                if "agent_configurations" in config_data:
                    self.cortex_config["agent_configurations"] = config_data[
                        "agent_configurations"
                    ]
        ...
        def update_variable():
            ...
            if not variable_name or new_value is None:
                return (
                    jsonify(
                        {
                            "status": "error",
                            "message": "Missing variable_name or new_value",
                        }
                    ),
                    400,
                )
            ...
            success = agent_variable_inspector.update_variable(
                variable_name, new_value, agent_name
            )
        ...
        def get_live_agents():
            ...
            return jsonify(
                {
                    "live_agents": list(
                        agent_variable_inspector.agent_instances.keys()
                    ),
                    "agent_details": {
                        name: {
        ...
        def import_variables():
            ...
            if success:
                ...
                return (
                    jsonify(
                        {"status": "error", "message": "Failed to import configuration"}
                    ),
                    400,
                )
            ...
            try:
                ...
                for tier, models in self.cortex.model_selector.model_registry.items():
                    model_stats[tier] = {
                        ...
                        "primary_provider": models.get("primary", {}).get(
                            "provider", "N/A"
                        ),
                        "cost_range": {
                            "min": min(
                ...
                status["configuration_source"] = (
                    str(self.db_path) if hasattr(self, "db_path") else "In-memory only"
                )
                status["agents_configured"] = len(
                    self.cortex_config["agent_configurations"]
                )
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            """,
                (
                    ...
                    config.get("model", ""),
                    config.get("provider", ""),
                    config.get("cost_per_1k", 0.0),
                    config.get("avg_latency", 0.0),
                    config.get("quality_score", 0.0),
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            """,
                (
                    agent_name,
                    config.get("tier", ""),
                    config.get("quality_threshold", 0.85),
                    config.get("max_cost_per_request", 0.1),
                    config.get("strategy_override"),
    ...
    async def _save_cortex_setting(
        self, key: str, value: str, setting_type: str, description: str = ""
    ):
            ...
            async with db.execute("SELECT * FROM agent_configurations") as cursor:
                async for row in cursor:
                    ...
                    config["agent_configurations"][agent_name] = {
                        "tier": row[2],
                        "quality_threshold": row[3],
                        "max_cost_per_request": row[4],
                        "strategy_override": row[5],
    ...
    async def _run_comprehensive_tests(self) -> Dict:
        ...
        test_scenarios = [
            {
                "name": "Executive High Complexity",
                "agent": "solomon",
                "complexity": 9,
                "quality": 0.95,
            },
            {
                "name": "Executive Low Complexity",
                "agent": "solomon",
                "complexity": 3,
                "quality": 0.80,
            },
        ...
        for scenario in test_scenarios:
            try:
                ...
                for strategy in SelectionStrategy:
                    selection = await self.cortex.model_selector.select_optimal_model(
        ...
        if test_results["performance_summary"].get("success_rate", 0) < 90:
            test_results["recommendations"].append(
                "Consider adjusting model tier configurations for better constraint satisfaction"
            )

        if test_results["performance_summary"].get("avg_cost_per_request", 0) > 0.01:
            test_results["recommendations"].append(
                "High average cost detected - consider cost optimization strategies"
            )
                ...
                if isinstance(value, dict):
                    for sub_key, sub_value in value.items():
                        ...
                        if var_name in agent_variable_inspector.variable_definitions:
                            agent_variable_inspector.variable_definitions[
                                var_name
    ...
    def _check_budget_alerts(
        self, daily_cost: float, monthly_cost: float
    ) -> List[Dict]:
