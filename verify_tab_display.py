...
def verify_tab_display():
    ...
    for rule in css_rules:
        ...
        if 'display: none !important' in rule and '.active' not in rule:
            print("  ⚠️  This rule uses !important which could prevent tabs from showing")
            problematic = True
    ...
    show_tab = re.search(r'function showTab\(tabName\)\s*{([^}]+(?:{[^}]*}[^}]*)*)}', content)
    if show_tab:
    ...
    tabs = ['dashboard', 'agents', 'leaders', 'departments', 'divisions', 'services', 'database', 'registry', 'system']

    for tab in tabs:
        tab_match = re.search(rf'<div id="{tab}" class="tab-content[^"]*">(.{{0,100}})', content, re.DOTALL)
        if tab_match:
    ...
    for tab in tabs:
        tab_section = re.search(rf'<div id="{tab}" class="tab-content[^>]*>.*?(?=<div id="|</script>)', content, re.DOTALL)
        if tab_section:
            section = tab_section.group()
            open_divs = section.count('<div')
            close_divs = section.count('</div>')
            if open_divs != close_divs:
                print(f"  ⚠️  {tab}: Unbalanced divs (open: {open_divs}, close: {close_divs})")
