...
from core.agent_cortex_langgraph_orchestrator import AgentCortexLangGraphOrchestrator, get_orchestrator
...
class TestSolomonAgent(EnhancedBaseAgent):
    ...
    async def think(self, context):
        ...
        return f"Solomon thinking about: {context.get('user_input', 'general situation')}"
...
async def test_cortex_core():
    ...
    try:
        ...
        test_request = AgentRequest(
            ...
            context={"user_input": "What's the best strategy for scaling our AI agents?"},
            complexity=7,
        ...
        logger.info(f"✅ Agent Cortex selected model: {cortex_response.selection.selected_model}")
        logger.info(f"✅ Agent Cortex reasoning: {cortex_response.selection.reasoning}")
        
        # Test LLM instance
        test_prompt = "Hello, this is a test prompt for the Agent Cortex-selected model."
        try:
...
async def test_langgraph_orchestrator():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"⚠️ Orchestrator processing failed (expected if no API keys): {e}")
...
async def test_integration():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✅ Agent Cortex selections made: {len(result.get('cortex_selections', []))}")
            logger.info(f"✅ Processing time: {result.get('processing_time', 0):.2f}s")
            
        except Exception as e:
            logger.warning(f"⚠️ Integration processing failed (expected if no API keys): {e}")
