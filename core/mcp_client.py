...
class MCPClient:
    ...
    async def _make_request(
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if result.get("error"):
                    raise MCPError(
                        result["error"]["message"], result["error"].get("code", -1)
                    )
    ...
    async def save_agent(
        self, name: str, config: Dict[str, Any], code: str = None
    ) -> Dict[str, Any]:
    ...
    async def save_memory(
        ...
        return await self._make_request(
            "memory.save", {"agent": agent_name, "memory": memory}
        )

    async def search_memories(
        self, query: str, agent_name: str = None, semantic: bool = True
    ) -> Dict[str, Any]:
    ...
    async def list_memories(
        self, agent_name: str, limit: int = 10, offset: int = 0
    ) -> Dict[str, Any]:
        """List memories for an agent"""
        return await self._make_request(
            "memory.list", {"agent": agent_name, "limit": limit, "offset": offset}
        )

    async def delete_memory(self, agent_name: str, memory_id: str) -> Dict[str, Any]:
        """Delete a specific memory"""
        return await self._make_request(
            "memory.delete", {"agent": agent_name, "id": memory_id}
        )
    ...
    async def batch_operations(
        ...
        for i, op in enumerate(operations):
            batch_ops.append(
                {"method": op["method"], "params": op["params"], "id": i + 1}
            )
...
async def quick_save_memory(
    agent_name: str, content: str, memory_type: str = "observation"
):
