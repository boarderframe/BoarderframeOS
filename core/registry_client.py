...
class RegistryClient:
            ...
            async with self.session.get(
                ...
                if response.status != 200:
                    raise ConnectionError(
                        f"Registry health check failed: {response.status}"
                    )
            ...
            try:
                async with self.session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.info(
                            f"Successfully registered {entity.type}: {entity.name}"
                        )
    ...
    async def discover_leaders(
        ...
        """Discover leaders"""
        entities = await self._discover(
            RegistryType.LEADER, department_id=department_id, division_id=division_id
        )
    ...
    async def discover_departments(
        self, division_id: Optional[str] = None, status: Optional[str] = None
    ) -> List[DepartmentRegistryEntry]:
        """Discover departments"""
        entities = await self._discover(
            RegistryType.DEPARTMENT, division_id=division_id
        )
    ...
    async def discover_servers(
        ...
        entities = await self._discover(
            registry_type, status=status, capability=capability
        )

        return [e for e in entities if isinstance(e, ServerRegistryEntry)]

    async def discover_databases(
        self, db_type: Optional[str] = None, status: Optional[ServiceStatus] = None
    ) -> List[DatabaseRegistryEntry]:
        """Discover databases"""
        entities = await self._discover(RegistryType.DATABASE, status=status)
    ...
    async def _discover(
        self, entity_type: RegistryType, **filters
        ...
        try:
            async with self.session.get(
                f"{self.config.registry_url}/discover/{entity_type}", params=params
            ) as response:
                if response.status == 200:
                    data = await response.json()
    ...
    async def update_entity(self, entity_id: str, updates: Dict[str, Any]):
        ...
        try:
            async with self.session.put(
                f"{self.config.registry_url}/entity/{entity_id}", json=updates
            ) as response:
                if response.status == 200:
                    # Invalidate cache
    ...
    async def unregister(self, entity_id: Optional[str] = None):
        ...
        entity_id = entity_id or (
            self._registered_entity.id if self._registered_entity else None
        )
        if not entity_id:
                ...
                async with websockets.connect(self.config.websocket_url) as websocket:
                    ...
                    if entity_types:
                        await websocket.send(
                            json.dumps(
                                {"command": "subscribe", "entity_types": entity_types}
                            )
                        )
    ...
    async def assign_agent_to_department(self, agent_id: str, department_id: str):
        ...
        try:
            async with self.session.post(
                ...
                if response.status == 200:
                    logger.info(
                        f"Successfully assigned agent {agent_id} to department {department_id}"
                    )
                else:
                    error = await response.text()
...
async def quick_register_agent(
    name: str, capabilities: Optional[List[str]] = None, **kwargs
) -> AgentRegistryEntry:
...
async def quick_discover_agents(
    capability: Optional[str] = None, department_id: Optional[str] = None
) -> List[AgentRegistryEntry]:
    """Quick function to discover agents"""
    async with RegistryClient() as client:
        return await client.discover_agents(
            capability=capability, department_id=department_id
        )


async def quick_register_server(
    name: str, server_type: ServerType, port: int, **kwargs
) -> ServerRegistryEntry:
    """Quick function to register a server"""
    async with RegistryClient() as client:
        return await client.register_server(
            name=name, server_type=server_type, host="localhost", port=port, **kwargs
        )
