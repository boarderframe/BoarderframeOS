...
def verify_agents_page():
    ...
    checks = {
        "✅ Using centralized metrics for active agents": 
            "active_agents = self.get_metric('agents', 'active')" in content,
        
        "✅ Inactive agents calculation added": 
            "inactive_agents = total_agents - active_agents" in content,
        
        "✅ 3-column grid for metrics": 
            re.search(r'Agent Metrics Grid.*?repeat\(3, 1fr\)', content, re.DOTALL) is not None,
        
        "✅ Active metric widget exists": 
            '<span>Active</span>' in content and '{active_agents}' in content,
        
        "✅ Inactive metric widget exists": 
            '<span>Inactive</span>' in content and '{inactive_agents}' in content,
        
        "✅ Total uses self.total_agents or fallback": 
            '{self.total_agents if hasattr(self, \'total_agents\') else total_agents}' in content,
        
        "✅ Overall status calculation added": 
            "overall_status_text = 'All Agents Offline'" in content,
        
        "✅ Status description updated": 
            "{active_agents} of {total_agents} agents active" in content,
        
        "✅ Filter simplified to Active/Inactive": 
            '<option value="inactive">🔴 Inactive</option>' in content and
            '<option value="productive">🚀 Productive</option>' not in content
    }
    ...
    metric_section = content[content.find("def generate_dashboard_html"):content.find("def generate_dashboard_html") + 3000]
